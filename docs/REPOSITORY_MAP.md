# Repository Map
## Purpose
This document is the navigation map for the public CRT research repository.
It is not a theory document.
It is not a proof artifact.
It is a guide to repository structure, status systems, and reading paths.
## Root
- [Repository root](../README.md)
- [Citation metadata](../CITATION.cff)
- [Publication status](../PUBLICATION_STATUS.md)
- [Theory index — legacy technical archive](../THEORY_INDEX.md)
- [Status glossary — legacy terminology](../STATUS_GLOSSARY.md)
## Core public documents
- [Architecture](ARCHITECTURE.md)
- [Architecture deep scan](ARCHITECTURE_DEEP_SCAN_v0_1.md)
- [Status](STATUS.md)
- [Status labels](STATUS_LABELS.md)
- [Claims ledger](CLAIMS_LEDGER.md)
- [Open problems](OPEN_PROBLEMS.md)
- [Reading guide](READING_GUIDE.md)
- [Research process](RESEARCH_PROCESS.md)
- [Terminology](TERMINOLOGY.md)
- [Legacy archive guide](LEGACY_ARCHIVE.md)
- [Rebuild audit](REBUILD_AUDIT_v0_1.md)
## Disclosure and governance
- [AI usage](AI_USAGE.md)
- [Authorship and privacy](AUTHORSHIP_AND_PRIVACY.md)
- [Review policy](REVIEW_POLICY.md)
- [Release policy](RELEASE_POLICY.md)
- [Publication boundary](PUBLICATION_BOUNDARY.md)
- [Claim labels](CLAIM_LABELS.md)
## Workflow and audit protocols
- [Codex workflow](../CODEX_WORKFLOW.md)
- [Role model](ROLE_MODEL.md)
- [Audit protocol](AUDIT_PROTOCOL.md)
- [Adversarial theory development protocol](ADVERSARIAL_THEORY_DEVELOPMENT_PROTOCOL.md)
- [Proof publication policy](PROOF_PUBLICATION_POLICY.md)
- [Methodology retrospective](METHODOLOGY_RETROSPECTIVE_v0_1.md)
- [Methodology case study](METHODOLOGY_CASE_STUDY_v0_1.md)
- [Development timeline](DEVELOPMENT_TIMELINE_v0_1.md)
- [Founding principles](FOUNDING_PRINCIPLES.md)
- [Emergent research process](EMERGENT_RESEARCH_PROCESS.md)
## AI-agent memory
- [AI agent instructions](../AGENTS.md)
- [AI agent context memo](AI_AGENT_CONTEXT.md)
## Public artifacts
- [Public artifacts](../artifacts/README.md)
- [Zenodo records](../artifacts/zenodo/README.md)
- [Zenodo 19884063](../artifacts/zenodo/19884063/README.md)
- [Zenodo 19937666](../artifacts/zenodo/19937666/README.md)
- [Public release model](PUBLIC_RELEASE_MODEL.md)
## Theory layer entry points
- [MST](../mst/README.md)
- [CRT](../crt/README.md)
- [IET](../iet/README.md)
## Proof artifacts
- [Proof artifacts](../proofs/README.md)
- [Proof status](../proofs/STATUS.md)
- [Proof artifact status template](../proofs/ARTIFACT_STATUS_TEMPLATE.md)
- [Proof supplement status](../proofs/supplement/STATUS.md)
Proof taxonomy:
- [Closed proof artifacts](../proofs/closed/README.md)
- [Conditional proof artifacts](../proofs/conditional/README.md)
- [Proof sketches](../proofs/sketches/README.md)
- [Audit artifacts](../proofs/audits/README.md)
- [Superseded proof artifacts](../proofs/superseded/README.md)
## Applications
- [Applications](../applications/README.md)
- [Institutional applications](../applications/institutional/README.md)
## Releases and archive
- [Releases](../releases/README.md)
- [Architecture release — 2026-05](../releases/architecture-release-2026-05.md)
- [Archive](../archive/README.md)
## Legacy paper and technical paths
The repository may still contain older paper scaffolds and technical archive files.
Read legacy material through:
- [Legacy archive guide](LEGACY_ARCHIVE.md)
- [Status labels](STATUS_LABELS.md)
- [Claims ledger](CLAIMS_LEDGER.md)
- [Proof status](../proofs/STATUS.md)
Legacy material should not be treated as the current public entry path unless explicitly stated.
